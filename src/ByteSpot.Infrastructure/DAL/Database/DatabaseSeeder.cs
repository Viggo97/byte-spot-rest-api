...
internal sealed class DatabaseSeeder(ByteSpotDbContext dbContext)
{
    ...
    private async Task AddEmploymentTypeTranslations()
    {
        if (!dbContext.EmploymentTypeTranslations.Any())
        {
            var employmentTypeTranslations = new List<EmploymentTypeTranslation>()
            {
        ...
        List<ExperienceLevel> experienceLevels, List<EmploymentType> employmentTypes)
    {
        ...
        {
            var employmentType = employmentTypes.SingleOrDefault(t => t.Value == employmentTypeName);
